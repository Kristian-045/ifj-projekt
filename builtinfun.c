...
void convert_int2_float() {
    ...
    printf("INT2FLOAT LF@retval1 LF@input\n");

    //return the converted value
    //printf("MOVE LF@retval1 LF@output\n");
...
void convert_float2_int() {
    ...
    printf("FLOAT2INT LF@retval1 LF@input\n");
...
void fun_length() {
    ...
    printf("MOVE LF@retval1 LF@result\n");
...
void fun_concat() {
    ...
    printf("MOVE LF@str1 LF@arg1\n");
    printf("MOVE LF@str2 LF@arg2\n");
...
void fun_substring() {
   printf("JUMP $substring_end\n");            
    ...
    printf("MOVE LF@string LF@arg1\n");
    printf("MOVE LF@start LF@arg2\n");
    printf("MOVE LF@end LF@arg3\n");
    ...
    printf("LT LF@boolval LF@start LF@length\n");
    printf("JUMPIFEQ $substring_error LF@boolval bool@false\n");
...
void fun_strcmp() {
    ...
    printf("STRLEN LF@len1 LF@arg1\n");            
    printf("STRLEN LF@len2 LF@arg2\n");             

    //loop to compare characters of both strings
    printf("LABEL $strcmp_loop\n");
    printf("LT LF@retval1 LF@i LF@len1\n");        // check if index < length of s1
    printf("JUMPIFEQ $strcmp_end1 LF@retval1 bool@false\n");
    printf("LT LF@retval1 LF@i LF@len2\n");        // check if index < length of s2
    printf("JUMPIFEQ $strcmp_end1 LF@retval1 bool@false\n");

    printf("GETCHAR LF@char1 LF@arg1 LF@i\n");      // get character at index i from s1
    printf("GETCHAR LF@char2 LF@arg2 LF@i\n");      // get character at index i from s2

    // compare the current characters
    printf("JUMPIFEQ $strcmp_next LF@char1 LF@char2\n");  // characters are equal
    printf("LT LF@retval1 LF@char1 LF@char2\n");   // check if char1 < char2
    printf("JUMPIFEQ $strcmp_smaller LF@retval1 bool@true\n");
    ...
    // end of comparison
    printf("LABEL $strcmp_end1\n");
    printf("LT LF@retval1 LF@len1 LF@len2\n");     // check if s1 is shorter than s2
    printf("JUMPIFEQ $strcmp_smaller LF@retval1 bool@true\n");
    printf("GT LF@retval1 LF@len1 LF@len2\n");     // check if s1 is longer than s2
    printf("JUMPIFEQ $strcmp_greater LF@retval1 bool@true\n");
...
void fun_ord_value() {
    ...
    printf("STRLEN LF@strlen LF@arg1\n");       

    // check if the string is empty or the index is out of limit
    printf("LT LF@cont LF@arg2 int@0\n");       
    printf("JUMPIFEQ $ord_error LF@cont bool@true\n");
    printf("LT LF@cont LF@arg2 LF@strlen\n");
    printf("JUMPIFEQ $ord_error LF@cont bool@false\n");

    // extract the character at the specified index
    printf("STRI2INT LF@retval1 LF@arg1 LF@arg2\n"); 
...
void fun_char() {
    ...
    printf("LT LF@check LF@arg1 int@0\n");       
    printf("JUMPIFEQ $char_error LF@check bool@true\n");
    printf("GT LF@check LF@arg1 int@255\n");
    printf("JUMPIFEQ $char_error LF@check bool@true\n");

    // convert integer to character and store in result
    printf("INT2CHAR LF@retval1 LF@arg1\n");       // convert ASCII code to character
...
void string_function() {
    printf("JUMP $string_end\n");
    printf("LABEL $string\n");
    printf("PUSHFRAME\n");

    // Define the local variable to hold the output
    printf("DEFVAR LF@retval1\n");
    printf("MOVE LF@retval1 LF@arg1\n"); // Directly return the argument

    printf("POPFRAME\n");
    printf("RETURN\n");
    printf("LABEL $string_end\n");
}
